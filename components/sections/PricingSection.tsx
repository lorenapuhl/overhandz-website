"use client"
// "use client" needed: uses Framer Motion for scroll and hover animations.

import { motion } from "framer-motion"
import SectionWrapper from "@/components/ui/SectionWrapper"
import Badge from "@/components/ui/Badge"
import Button from "@/components/ui/Button"
import { pricingPlans } from "@/components/data/pricing"

// ---------------------------------------------------------------------------
// PricingSection — 3 pricing tier cards (Drop-in, Class Pack, Membership)
//
// The highlighted card (Class Pack) is visually elevated:
//   - White border instead of dim border
//   - "Best Value" badge
//
// Used on both the landing page and the /pricing page.
//
// // FUTURE BACKEND:
// // Replace pricingPlans mock data with Stripe Product/Price API
// // Add checkout → integrate Stripe Checkout or Stripe Payment Links
// ---------------------------------------------------------------------------

export default function PricingSection() {
  return (
    <section className="py-20 px-6 md:py-36 md:px-12 bg-canvas">
      <SectionWrapper>

        {/* SECTION HEADER */}
        <motion.div
          whileInView={{ opacity: 1, y: 0 }}
          initial={{ opacity: 0, y: 20 }}
          viewport={{ once: true }}
          transition={{ duration: 0.4, ease: "easeOut" }}
          className="text-center mb-12"
        >
          <p className="text-dim text-sm font-medium tracking-widest uppercase mb-2">
            Simple pricing
          </p>
          <h2 className="text-white font-semibold text-3xl md:text-5xl tracking-tight">
            Train on your terms
          </h2>
          <p className="text-dim text-base mt-4 max-w-md mx-auto">
            No contracts, no hidden fees. Pick what fits your schedule.
          </p>
        </motion.div>

        {/* PRICING CARDS — 3 columns on desktop, stacked on mobile */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-6">
          {pricingPlans.map((plan, index) => (
            <motion.div
              key={plan.id}
              whileInView={{ opacity: 1, y: 0 }}
              initial={{ opacity: 0, y: 30 }}
              viewport={{ once: true }}
              transition={{ duration: 0.4, ease: "easeOut", delay: index * 0.1 }}
              // The highlighted plan gets a different border color for visual emphasis
              // ring-1 ring-white = subtle white border on the featured card
              className={`
                bg-surface rounded-xl p-6 flex flex-col
                ${plan.highlight
                  ? "border border-white/40 ring-1 ring-white/10"
                  : "border border-edge"
                }
              `}
            >
              {/* CARD TOP — plan name + optional badge */}
              <div className="flex items-start justify-between mb-6">
                <div>
                  <p className="text-white font-semibold text-lg">{plan.name}</p>
                  <p className="text-dim text-sm mt-1">{plan.description}</p>
                </div>
                {/* Badge only shown on highlighted (best value) plan */}
                {plan.badge && (
                  <Badge className="shrink-0 ml-2">{plan.badge}</Badge>
                )}
              </div>

              {/* PRICE */}
              <div className="mb-6">
                <span className="text-white font-bold text-4xl">€{plan.price}</span>
                <span className="text-dim text-sm ml-2">{plan.period}</span>
              </div>

              {/* BENEFITS LIST */}
              <ul className="space-y-3 mb-8 flex-1">
                {plan.benefits.map((benefit) => (
                  <li
                    key={benefit.text}
                    className="flex items-start gap-3 text-sm"
                  >
                    {/* Checkmark or X depending on whether benefit is included */}
                    <span
                      className={`mt-0.5 shrink-0 ${
                        benefit.included ? "text-white" : "text-edge"
                      }`}
                    >
                      {benefit.included ? (
                        // Check icon
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
                          <polyline points="20 6 9 17 4 12" />
                        </svg>
                      ) : (
                        // Minus icon for not-included items
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
                          <line x1="5" y1="12" x2="19" y2="12" />
                        </svg>
                      )}
                    </span>
                    <span className={benefit.included ? "text-dim" : "text-edge"}>
                      {benefit.text}
                    </span>
                  </li>
                ))}
              </ul>

              {/* CTA BUTTON */}
              <Button
                href="/schedule"
                variant={plan.highlight ? "primary" : "secondary"}
                className="w-full"
              >
                {plan.cta}
              </Button>
            </motion.div>
          ))}
        </div>
      </SectionWrapper>
    </section>
  );
}
